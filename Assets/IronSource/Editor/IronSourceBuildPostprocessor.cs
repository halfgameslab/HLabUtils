...
namespace IronSource.Editor
{
	public class IronSourceBuildPostprocessor
	{
		...
		public static void OnPostprocessBuild (BuildTarget buildTarget, string buildPath)
		{
			if (buildTarget == BuildTarget.iOS) {
				...
				if (Directory.Exists (dirpath)) {
					...
					var files = Directory.GetFiles (dirpath, "*.cs", SearchOption.TopDirectoryOnly).Where (file => Regex.IsMatch (System.IO.Path.GetFileName (file), "^(?!I).+Settings.*$"));
		...
		private static void updateProject (BuildTarget buildTarget, string projectPath)
		{
			...
			string targetId = project.TargetGuidByName (PBXProject.GetUnityTestTargetName ());
