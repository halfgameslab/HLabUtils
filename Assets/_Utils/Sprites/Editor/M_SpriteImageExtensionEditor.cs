...
{
    [CustomEditor(typeof(M_SpriteImageExtension))]
    //[CanEditMultipleObjects]
    public class M_SpriteImageExtensionEditor : Editor
    {
        private static readonly string MISSING_STRING = "<< missing >>";
        private static readonly string[] EMPTY_STRING_ARRAY = new string[] { "<< Empty >>" };

        private M_SpriteImageExtension _currentTarget;
        ...
        {
            _currentTarget = ((M_SpriteImageExtension)target);
            Sprite currentSprite = _currentTarget.GetComponent<Image>().sprite;
